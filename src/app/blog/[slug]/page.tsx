...
export default async function BlogDetailPage({ params }: BlogDetailPageProps) {
  ...
  const { data: post, error } = await getPublishedPostBySlug(PostType.BLOG, slug);
      ...
      <article className="rounded-[2rem] border border-black/5 bg-white/85 p-8 shadow-[0_20px_70px_rgba(73,41,17,0.08)]">
        ...
        {post.category || post.tags.length > 0 ? (
          <div className="mt-6 flex flex-wrap gap-2">
            {post.category ? (
              <Link
                href={getCollectionHref(PostType.BLOG, { categorySlug: post.category.slug })}
            ...
            {post.tags.map((tag) => (
              <Link
                key={tag.id}
                href={getCollectionHref(PostType.BLOG, { tagSlug: tag.slug })}
